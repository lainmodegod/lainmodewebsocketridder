...
"""
LAINMODE WEBSOCKET RIDDLER - Universal WebSocket Security Testing Tool
...
# СРАЗУ ПЕЧАТАЕМ БАННЕР - ДО ВСЕХ ИМПОРТОВ!
print(BANNER)
print()

# ==============================================================================
#                           ИМПОРТЫ ПОСЛЕ БАННЕРА
# ==============================================================================
        ...
        try:
            ...
            if response:
                ...
                if vulnerable:
                    ...
                    # Специальный баннер для критических находок
                    print(f"\n{Colors.RED}{Colors.BOLD}╔════════════════════════════════════════════════════════════╗{Colors.END}")
                    print(f"{Colors.RED}{Colors.BOLD}║          🚨 LAINMODE DETECTED A VULNERABILITY! 🚨          ║{Colors.END}")
                    print(f"{Colors.RED}{Colors.BOLD}╠════════════════════════════════════════════════════════════╣{Colors.END}")
                    print(f"{Colors.RED}{Colors.BOLD}║  Target: {target}{' ' * (45 - len(str(target)))}║{Colors.END}")
                    print(f"{Colors.RED}{Colors.BOLD}║  Type: {target_type}{' ' * (45 - len(target_type))}║{Colors.END}")
                    print(f"{Colors.RED}{Colors.BOLD}║  Reason: {reason[:40]}{' ' * (43 - min(40, len(reason)))}║{Colors.END}")
                    print(f"{Colors.RED}{Colors.BOLD}╚════════════════════════════════════════════════════════════╝{Colors.END}")
  ...
  python3 lainmodewebsocketridder.py --generate-config > my_config.yaml
    ...
    if args.config:
        ...
        if not args.url or not args.token:
            print(f"\n{Colors.error('URL and token required without config file')}")
            print(f"{Colors.info('Use: python3 lainmodewebsocketridder.py -u URL -t TOKEN')}")
            print(f"{Colors.info('Or: python3 lainmodewebsocketridder.py -c config.yaml')}")
            print(f"{Colors.info('Or: python3 lainmodewebsocketridder.py --generate-config')}")
            return
